...
public class Puppet extends RigidBody {
    ...
    public void makeItAlive(boolean mover, float angulo, boolean saltar, float dt)
    {
        ...
        {
            float[] ubi= getWorldLocationClone();
    ...
    public void testShot(float dt, Entity p)
    {
        beeptimer-=dt;
        if(beeptimer<=0)
        {
            ...
            for(int i = 0; i< SceneManager.activeSceneManager.scenePhoto.size(); i++)
            {
                ...
                {
                    float[] o= SceneManager.activeSceneManager.viewPort.getWorldLocationClone();
                    float[] u= SceneManager.activeSceneManager.viewPort.getDirectionZn();
                    float[] ubi=new float[3];

                    if(SceneManager.activeSceneManager.phy.testBodyRay(SceneManager.activeSceneManager.scenePhoto,i,o,u,ubi))
                    {
                        p.setLocation(ubi[0], ubi[1], ubi[2]);
