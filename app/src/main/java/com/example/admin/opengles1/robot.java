...
public class robot extends puppet {
    ...
    public ArrayList<String> idleAction=new ArrayList<String>();
    prg todo;
    // if exp <comp> exp { }
    // let var=exp
    // add var,exp
    ...
    final String script =
            "if estado=0 { let wt=time();add wt,15;wait parado;add estado,1;}"+
            "if estado=1 { if time()>wt {goto 8,20; add estado,1}}"+
            "if estado=2 { if dist()<0.5 {let wt=time();add wt,15;wait parado,Mirar,Tirar;add estado,1;}}"+
            "if estado=3 { if time()>wt {goto -3,2; add estado,1}}"+
            "if estado=4 { if dist()<0.5 {let wt=time();add wt,15;wait Tirar,Mirar;add estado,1;}}"+
            "if estado=5 { if time()>wt {goto -6,17; add estado,1}}"+
            "if estado=6 { if dist()<0.5 {let wt=time();add wt,15;wait Mirar;set estado=1;}}";
    ...
    public void makeItAlive(float dt)
    {
        ...
        {
            float[] mat=getWorldMatrix();   // matriz global del personaje
...
class prg{
    ...
    public prg(String the_code, robot rob)
    {
        code=the_code;
        commanded=rob;
        commands=new ArrayList<sentence>();
    ...
    float getSentenceValue(sentence sen, int parm)
    {
        ...
        if(sen.val[parm].contentType==valueType.token)
        {
            ...
            {
                float[] mat=commanded.getWorldMatrix();   // matriz global del personaje
